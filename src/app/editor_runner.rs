...
impl EditorRunner {
    ...
    pub fn new() -> Self {
        Self { bus: MessageBus::new(), state: EditorState::default(), last_time: Instant::now() }
    }
    ...
    fn render(&mut self, core: &mut AppCore) {
        ...
        let draw_cmds: Vec<DrawCommand> = core.world
            ...
            .collect();
        for cmd in draw_cmds { core.draw_queue.push(cmd); }
        ...
        let hierarchy: Vec<(Entity, String)> = {
            ...
            for root in roots {
                core.world.scene_tree().walk_depth_first(root, |e, depth| {
                    ...
                    let label  = core.world.get::<Tag>(e)
        ...
        let mut encoder = core.render_ctx.device.create_command_encoder(
            &wgpu::CommandEncoderDescriptor { label: Some("editor frame") },
        );
        ...
        // --- 6. imgui UI (NLL ensures ui borrow ends before end_frame) ---
        {
            ...
            ui.window("##toolbar")
                ...
                .build(|| {
                    if ui.button("Save") { save_req = true; }
                    ui.same_line();
                    if ui.button("Load") { load_req = true; }
                    ui.same_line();
                    if ui.button("Run Game") { run_game_req = true; }
                    ui.same_line();
                    if ui.button("+ Spawn") { spawn_req = true; }
                    ui.same_line();
                ...
                ui.window("Inspector")
                    ...
                    .build(|| {
                        ...
                        if let Some(ref mut tf) = new_transform {
                            ui.text("[ Transform ]");
                            if ui.input_float("px##tf", &mut tf.position.x).build() { transform_changed = true; }
                            if ui.input_float("py##tf", &mut tf.position.y).build() { transform_changed = true; }
                            if ui.input_float("rot##tf", &mut tf.rotation).build()  { transform_changed = true; }
                            if ui.input_float("sx##tf", &mut tf.scale.x).build()    { transform_changed = true; }
                            if ui.input_float("sy##tf", &mut tf.scale.y).build()    { transform_changed = true; }
                            if ui.small_button("Remove##rm_tf") { remove_transform = true; }
                            ui.separator();
                        ...
                        if let Some(ref mut c) = new_color {
                            ...
                            if ui.color_edit4("##col", &mut arr) {
                                c.r = arr[0]; c.g = arr[1]; c.b = arr[2]; c.a = arr[3];
                                color_changed = true;
                            }
                            if ui.small_button("Remove##rm_col") { remove_color = true; }
                            ui.separator();
                        } else if ui.button("+ Color") {
                            new_color = Some(Color { r: 1.0, g: 1.0, b: 1.0, a: 1.0 });
                            color_changed = true;
                        }
                        ...
                        if let Some(ref mut sin) = new_sinusoid {
                            ui.text("[ Sinusoid ]");
                            if ui.slider("amp##sin",    0.0_f32, 500.0, &mut sin.amplitude)  { sinusoid_changed = true; }
                            if ui.slider("freq##sin",   0.0_f32,  10.0, &mut sin.frequency)  { sinusoid_changed = true; }
                            if ui.slider("phase##sin",       -PI,   PI, &mut sin.phase)       { sinusoid_changed = true; }
                            if ui.slider("base_y##sin", -400.0_f32, 400.0, &mut sin.base_y)  { sinusoid_changed = true; }
                            if ui.small_button("Remove##rm_sin") { remove_sinusoid = true; }
                            ui.separator();
                        } else if ui.button("+ Sinusoid") {
                            new_sinusoid = Some(SinusoidComponent {
                                amplitude: 100.0, frequency: 1.0, phase: 0.0, base_y: 0.0,
                            });
                            sinusoid_changed = true;
                        }

                        ui.separator();
                        if ui.button("Despawn Entity") { despawn_req = true; }
                    });
            }
        ...
        if cam_zoom != 0.0 { self.state.camera.zoom_toward(cam_zoom); }

        if let Some(entity) = selected {
            if remove_transform    { core.world.remove::<Transform>(entity); }
            else if transform_changed  { if let Some(tf) = new_transform { core.world.insert(entity, tf); } }

            if remove_color        { core.world.remove::<Color>(entity); }
            else if color_changed      { if let Some(c) = new_color     { core.world.insert(entity, c); } }

            if remove_sinusoid     { core.world.remove::<SinusoidComponent>(entity); }
            else if sinusoid_changed   { if let Some(s) = new_sinusoid  { core.world.insert(entity, s); } }
        }
        ...
        if run_game_req  { run_game(&mut self.state.status_message); }
...
fn run_game(status: &mut String) {
    ...
    let game_bin = std::env::current_exe()
        ...
        .map(|dir| {
            if cfg!(windows) { dir.join("game.exe") } else { dir.join("game") }
        });
